...
Client-Side:
  * DP-SGD (Differential Privacy)
    - Gradient clipping (max norm C)
    - Calibrated Gaussian noise injection
    - Reference: Abadi et al., CCS 2016
...
NOISE CALIBRATION (from literature):
====================================
  epsilon=1.0  -> noise_multiplier=0.5  (strong privacy, ~5-10% utility loss)
  epsilon=2.0  -> noise_multiplier=0.3  (moderate privacy, ~2-5% utility loss)
  epsilon=4.0  -> noise_multiplier=0.2  (relaxed privacy, ~1-3% utility loss)
  epsilon=8.0  -> noise_multiplier=0.1  (minimal privacy, ~0-2% utility loss)
...
USAGE:
    ...
    python p3_train_federated_secure.py --epsilon 1.0 --rounds 30
...
# ============================================================================
# DP-SGD OPTIMIZER - Abadi et al., CCS 2016
# ============================================================================
...
class DPOptimizer:
    ...
    Noise calibration based on literature:
    - Abadi et al. (CCS 2016): sigma=1.1 for epsilon~2-8 on MNIST
    - McMahan et al. (ICLR 2018): sigma in [0.3, 1.0] for practical FL
    - Apple/Google deployments: sigma~0.5-1.0 with epsilon in [4, 10]
    """
    ...
    def step(self, model: nn.Module) -> float:
        """Perform DP-SGD step: clip -> add noise -> update."""
...
class FedValDefense:
    ...
    def __init__(self, threshold: float = 0.1, validation_data: Data = None):
...
class SecureFederatedClient:
    """Federated Learning Client with DP-SGD and Adversarial Training."""
    ...
    def __init__(
        ...
        self.adv_ratio = adv_ratio if not use_dp else min(adv_ratio, 0.1)
    ...
    def train(self, alpha_forward: float = 1.0, alpha_backward: float = 1.5) -> Tuple[List[np.ndarray], int, Dict]:
        ...
        base_optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr)
        ...
        for epoch in range(self.local_epochs):
            ...
            if self.use_dp:
                grad_norm = dp_optimizer.step(self.model)
        ...
        return self.get_parameters(), int(train_mask.sum()), metrics
...
class SecureFederatedServer:
    """Federated Server with FLAME and FedVal defenses."""
...
def run_secure_fl(
    ...
    print(f"\n  SECURITY SETTINGS:")
    print(f"    DP-SGD: {'Enabled' if use_dp else 'Disabled'}")
    if use_dp:
...
def main(args):
    ...
    if not args.no_dp:
        ...
        print(f"\n  Privacy: epsilon={args.epsilon}, noise_multiplier={noise_mult}")
    ...
    torch.save({
        ...
        'security_config': {
            ...
            'fedval_enabled': not args.no_fedval
        },
        'fl_config': {
            'rounds': args.rounds,
            'local_epochs': args.local_epochs,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epsilon', type=float, default=2.0,
                        help='Target epsilon (default=2.0 for moderate privacy)')
    ...
    parser.add_argument('--no-fedval', action='store_true')
