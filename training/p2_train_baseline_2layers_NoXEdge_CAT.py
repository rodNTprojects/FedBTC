...
def create_pyg_data(data: pd.DataFrame, edges: pd.DataFrame,
                    seed: int = 42, no_cross_edges: bool = True) -> Data:
    """
    ...
    longer affects the split (which is now canonical and seed-independent).
    """
    print("\n" + "=" * 60)
    ...
    # NoXEdge: remove cross-exchange edges
    if no_cross_edges and edge_index.shape[1] > 0:
        node_exchange = forward_labels.numpy()
        src_exch = node_exchange[edge_index[0].numpy()]
        dst_exch = node_exchange[edge_index[1].numpy()]
        keep_mask = torch.tensor(src_exch == dst_exch)
        removed = int((~keep_mask).sum())
        edge_index = edge_index[:, keep_mask]
        print(f"  [NoXEdge] Removed {removed:,} cross-exchange edges")
...
def train(args):
    """CAT — Centralized Adversarial Training (NoXEdge + PGD-7)"""
    import random
    print("\n" + "="*70)
    print("CAT — CENTRALIZED ADVERSARIAL TRAINING (NoXEdge, PGD-7)")
    print("="*70)
    ...
    model_path   = f'results/models/baseline_dual_attribution_noxedge_cat{_tag}_seed{args.seed}.pt'
    results_path = f'results/evaluations/baseline_results_noxedge_cat{_tag}_seed{args.seed}.json'
    print(f'model_path  : {model_path}')
    print(f'results_path: {results_path}')
    # Load data
    data, edges = load_enriched_data(args.data_dir)
    _no_cross = not args.keep_cross_edges
    print(f'Topology: {"NoXEdge" if _no_cross else "AllEdges"}')
    graph_data = create_pyg_data(data, edges, seed=args.seed, no_cross_edges=_no_cross)
    ...
    # CAT: PGD adversarial trainer (self-contained — no external dependency)
    # Uses compute_multitask_loss already defined in this script
    class _AT:
        def __init__(s, eps, steps, ratio):
            s.epsilon=eps; s.steps=steps; s.ratio=ratio; s.alpha=eps/4.0
        def generate(s, m, x, ei, yf, yb, mask, lc, bw):
            m.eval()
            xa = x.clone().detach()
            xa = xa + torch.empty_like(xa).uniform_(-s.epsilon, s.epsilon)
            xa = torch.clamp(xa, -10., 10.)
            for _ in range(s.steps):
                xa.requires_grad_(True)
                out = m(xa, ei)
                ot = {'forward': out['forward'][mask],
                      'backward': out['backward'][mask],
                      'embeddings': out['embeddings'][mask]}
                lb = {'forward': yf[mask], 'backward': yb[mask]}
                l, _ = compute_multitask_loss(ot, lb, lc, bw)
                m.zero_grad(); l.backward()
                with torch.no_grad():
                    xa = xa + s.alpha * xa.grad.sign()
                    d  = torch.clamp(xa - x, -s.epsilon, s.epsilon)
                    xa = torch.clamp(x + d, -10., 10.).detach()
            m.train(); return xa

    adv_trainer = None
    if args.enable_at:
        adv_trainer = _AT(args.at_epsilon, args.at_steps, args.at_ratio)
        print(f'CAT enabled: PGD-{args.at_steps} eps={args.at_epsilon} ratio={args.at_ratio}')
    else:
        print('CAT disabled — pass --enable_at to activate')
    ...
    for epoch in range(args.epochs):
        ...
        # Forward pass — clean or adversarial (CAT)
        train_mask = graph_data_d.train_mask
        if adv_trainer is not None and torch.rand(1).item() < adv_trainer.ratio:
            x_in = adv_trainer.generate(
                model, graph_data_d.x, graph_data_d.edge_index,
                graph_data_d.y_forward, graph_data_d.y_backward,
                train_mask, loss_config, backward_weights)
        else:
            x_in = graph_data_d.x
        output = model(x_in, graph_data_d.edge_index)
        labels = {
...
if __name__ == "__main__":
    ...
    parser.add_argument('--enable_at', action='store_true',
                        help='Enable Centralized Adversarial Training (CAT)')
    parser.add_argument('--at_epsilon', type=float, default=0.2,
                        help='PGD perturbation budget')
    parser.add_argument('--at_steps', type=int, default=7,
                        help='PGD steps')
    parser.add_argument('--at_ratio', type=float, default=0.30,
                        help='Fraction of epochs using adv samples')
