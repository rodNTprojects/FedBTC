"""
================================================================================
Script: p2_train_baseline_2layers_NoXEdge.py
Description: BASELINE ABLATION — No Cross-Exchange Edges

PURPOSE:
  Isolate the effect of graph partitioning on FL vs Centralized comparison.
  This script is identical to p2_train_baseline_2layers.py EXCEPT that
  cross-exchange edges are removed before training.
  
  If Centralized-NoXEdge ≈ FL, then the FL advantage comes from graph
  partitioning (fewer edges → less over-smoothing), not from FedAvg itself.
  
  If FL > Centralized-NoXEdge, then FedAvg provides additional value
  beyond graph partitioning (e.g. implicit regularization).

USAGE:
    python p2_train_baseline_2layers_NoXEdge.py --seed 42
    python p2_train_baseline_2layers_NoXEdge.py --no_cross_edges --seed 42
    
    # To run WITH cross-edges (same as original baseline):
    python p2_train_baseline_2layers_NoXEdge.py --keep_cross_edges --seed 42
================================================================================
"""
...
# Default seed (overridden by --seed argument)
...
# Feature patterns per category: (mean, std) pour chaque feature discriminative
CATEGORY_PATTERNS = {
...
class DualAttributionModel(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        # Simplified because fewer classes
...
def compute_multitask_loss(
    ...
    IMPORTANT: Filter NO_MERCHANT (label=0) for backward loss
    """
...
def load_enriched_data(data_dir: str = 'data/federated_enriched') -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for i in range(3):
        ...
        if os.path.exists(enriched_path):
            ...
            print(f"  [OK]“ Exchange {i}: {len(df):,} rows (enriched)")
        elif os.path.exists(original_path):
            df = pd.read_pickle(original_path)
            print(f"  [WARNING] Exchange {i}: {len(df):,} rows (original - no enrichment)")
        else:
...
def generate_discriminative_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'category_label' not in data.columns:
        print("    [ERROR] No category_label - cannot generate features")
        return data
    ...
    print(f"    [OK] Category features: {len(cat_features)}")
    
    return data

def create_pyg_data(data: pd.DataFrame, edges: pd.DataFrame, no_cross_edges: bool = True) -> Data:
    """
    Create PyTorch Geometric Data object — STRICT canonical mode (Option B-2).

    Reads canonical artifacts in read-only mode:
      - data/split_map.pkl
      - data/normalization_stats.pkl

    Preserves the no_cross_edges filter (NoXEdge ablation).
    """
    print("\n" + "=" * 60)
    print("CREATING PYTORCH GEOMETRIC DATA (canonical Option B-2)")
    print("=" * 60)

    # ===== HANDLE CATEGORY LABELS =====
    if 'category_label' in data.columns:
        print("  [OK] Using existing category_label")
        ...
        print("  [ERROR] No backward_label - creating dummy category_label")
    ...
    if not cat_features_exist:
        ...
        cat_features = [c for c in data.columns if c.startswith('cat_')]
        print(f"  [OK] Found {len(cat_features)} existing cat_* features")
    ...
    feat_original = sum(1 for c in feature_cols if c.startswith('feat_'))
    feat_cat      = sum(1 for c in feature_cols if c.startswith('cat_'))
    ...
    if edge_list:
        if no_cross_edges:
            exchange_labels = (data['forward_label'].values
                              if 'forward_label' in data.columns
                              else data['exchange_partition'].values)
            total_before = len(edge_list)
            edge_list = [
                [idx1, idx2] for idx1, idx2 in edge_list
                if exchange_labels[idx1] == exchange_labels[idx2]
            ]
            total_after = len(edge_list)
            cross_removed = total_before - total_after
            print(f"  [NO-XEDGE] Removed {cross_removed:,} cross-exchange edges "
                  f"({total_before:,} -> {total_after:,}, "
                  f"-{cross_removed/total_before*100:.1f}%)")
    ...
    graph_data.train_mask = torch.BoolTensor(train_mask)
    graph_data.val_mask   = torch.BoolTensor(val_mask)
    graph_data.test_mask  = torch.BoolTensor(test_mask)
    graph_data.num_features = X.shape[1]

    print(f"  [CANONICAL] Using split_map.pkl ({sum(train_mask)} train, "
          f"{sum(val_mask)} val, {sum(test_mask)} test)")
    print(f"  Splits: Train={graph_data.train_mask.sum()}, "
          f"Val={graph_data.val_mask.sum()}, Test={graph_data.test_mask.sum()}")
...
def train(args):
    ...
    no_xedge = not args.keep_cross_edges
    graph_data = create_pyg_data(data, edges, no_cross_edges=no_xedge)
    ...
    #Align backward weights with Federated Learning
    counts = Counter(train_backward_labels_filtered.cpu().numpy())
    total = sum(counts.values())
    weights = [0.0]  # NO_MERCHANT
    for i in range(1, 6):
        count = counts.get(i, 1)
        weights.append(min(total / (5 * count), 10.0))
    backward_weights = torch.tensor(weights, dtype=torch.float32, device=device)
    backward_weights = backward_weights / backward_weights[1:].mean()
    backward_weights[0] = 0.0
    ...
    for epoch in range(args.epochs):
        ...
        if combined > best_metric:
            ...
            }, f'results/models/baseline_dual_attribution{tag}_seed{args.seed}.pt')
            #improved = " â˜…"
            improved = " ★"
        else:
            patience_counter += 1
        ...
        # Early stopping (disabled for fair comparison with FL which always runs full budget)
        # To re-enable: set --patience to a value < epochs (e.g. --patience 20)
        if args.patience < args.epochs and patience_counter >= args.patience:
            print(f"\nEarly stopping at epoch {epoch}")
            break
    
    # Load best and evaluate on test
    checkpoint = torch.load(f'results/models/baseline_dual_attribution{tag}_seed{args.seed}.pt')
    ...
    print(f"\n{'[OK]' if fwd_ok else '[WARNING]'} Forward target (>=85%): {test_metrics['forward']['accuracy']:.2%}")
    print(f"{'[OK]' if bwd_ok else '[WARNING]'} Backward target (>=60%): {test_metrics['backward']['accuracy']:.2%}")
    ...
    results = {
        # === IDENTIFICATION ===
        'approach': 'baseline_noxedge' if no_xedge else 'baseline',
        'approach_name': 'Centralized Baseline (No Cross-Exchange Edges)' if no_xedge else 'Centralized Baseline',
        'training_type': 'centralized',
        'privacy_level': 'none',
        'privacy_description': 'No privacy - all data centralized' + (' - cross-exchange edges removed' if no_xedge else ''),
    ...
    results_path = f'results/evaluations/baseline_results{tag}_seed{args.seed}.json'
    model_path = f'results/models/baseline_dual_attribution{tag}_seed{args.seed}.pt'
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2, default=str)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Baseline Ablation - No Cross-Exchange Edges')
    ...
    parser.add_argument('--patience', type=int, default=60,
                        help='Early stopping patience (set >= epochs to disable)')
    parser.add_argument('--num_layers', type=int, default=2,
                        help='Number of GCN layers (default: 2, optimal per ablation)')
    parser.add_argument('--seed', type=int, default=42,
                        help='Random seed for reproducibility')

    parser.add_argument('--results_tag', type=str, default='noxedge',
                    help='Tag added to result filename (default: "noxedge")')
    
    # Ablation: cross-exchange edges
    parser.add_argument('--keep_cross_edges', action='store_true',
                        help='Keep cross-exchange edges (default: remove them)')
